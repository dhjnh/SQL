...
def load_kb(path: str = "kb") -> Dict[str, Any]:
    ...
    pl_edges: Dict[Tuple[str, str], float] = {}
    for r in _read_csv_dicts(pl_link_path):
        ...
        key = (a, b) if a <= b else (b, a)
        s = _safe_float(r.get("score"), 0.0)
        prev = pl_edges.get(key)
        if (prev is None) or (s > prev):
            pl_edges[key] = s

    pl_adjacency: Dict[str, List[Tuple[str, float]]] = {}
    for (a, b), s in pl_edges.items():
...
def _pl_concepts_from_tokens(tokens: Sequence[str], kb: Dict[str, Any]) -> Set[str]:
    pl_norm: Dict[str, str] = kb.get("pl_norm_map", {}) or {}
    pl_concept_map: Dict[str, str] = kb.get("pl_concept_map", {}) or {}
    out: Set[str] = set()
    for t in tokens:
        if not t:
            continue
        x = pl_norm.get(t, t)
        c = pl_concept_map.get(x) or pl_concept_map.get(t)
        if c:
            out.add(_norm_text(c))
    return out


def _pl_concept_link_score(a_pl: Set[str], b_pl: Set[str], kb: Dict[str, Any]) -> float:
    if not a_pl or not b_pl:
        return 0.0
    adj = kb.get("pl_adjacency", {}) or {}
    s = 0.0
    for c in a_pl:
        for other, sc in adj.get(c, []):
            if other in b_pl:
                s += _safe_float(sc, 0.0)
    return s
...
def score_pl_rows(rows: List[Dict[str, Any]], kb: Dict[str, Any], category: Any, subcategory: Any) -> Tuple[List[RowScore], int]:
    ...
    pl_concept_cache: Dict[Tuple[str, ...], Set[str]] = {}
    pl_near_cache: Dict[Tuple[Tuple[str, ...], Tuple[str, ...]], float] = {}
    ...
    def _pl_concept_cached(tokens: List[str]) -> Set[str]:
        key = tuple(tokens)
        if key not in pl_concept_cache:
            pl_concept_cache[key] = _pl_concepts_from_tokens(tokens, kb)
        return pl_concept_cache[key]

    def _pl_near_cached(a_pl: Set[str], b_pl: Set[str]) -> float:
        ka = tuple(sorted(a_pl))
        kb2 = tuple(sorted(b_pl))
        key = (ka, kb2) if ka <= kb2 else (kb2, ka)
        if key not in pl_near_cache:
            pl_near_cache[key] = _pl_concept_link_score(a_pl, b_pl, kb)
        return pl_near_cache[key]
    ...
    for i, row in enumerate(rows):
        ...
        pnc_norm = _prep_text_for_tokens(pnc_text).replace("-", "_").replace(" ", "_")
        pnc_norm = re.sub(r"_+", "_", pnc_norm).strip("_")
        pd_norm = _prep_text_for_tokens(part_desc_text).replace("-", "_").replace(" ", "_")
        pd_norm = re.sub(r"_+", "_", pd_norm).strip("_")
        ...
        if kb.get("pl_enabled"):
            ...
            # --- NEW: PL concept graph small adjustment ---
            if (kb.get("pl_adjacency") and kb.get("pl_concept_map")) and (c_state != "HARD-FAIL") and (d_state != "HIT"):
                # old side: gpg_tokens（已清洗/去stopword/做pl_norm）
                old_pl = _pl_concept_cached(gpg_tokens)

                # new side: 优先 SubCategory label，其次 cat_sub 组合，再兜底 Category label
                new_keys = [sub_n, f"{cat_n}_{sub_n}", cat_n]
                new_pl = _pl_concept_cached([k for k in new_keys if k])

                pl_near = _pl_near_cached(old_pl, new_pl)

                # 保险：只做小幅加分；OUT 不允许 +2，避免被推过 like>=15 的门槛
                if pl_near >= 0.35:
                    context_adjust += (2 if d_state == "NEAR" else 1)
                elif pl_near >= 0.20:
                    context_adjust += 1
...
def pick_ispick(rows: List[RowScore], global_picked_spn: Set[str]) -> None:
    ...
    fill(tier1)
    fill(tier2)
    if len(picked) < PL_PICK_MAX:
        fill(tier3, allow_t3=True)

    for p in picked[:PL_PICK_MAX]:
        p.row["_ispick"] = 1
...
class App:
    def __init__(self, root: tk.Tk):
        ...
        self.clock_ema_samples = 0
        self.clock_eta_last_sec = None
        self.clock_recent = deque(maxlen=60)
        self.clock_recent_sec_sum = 0.0
        self.clock_recent_work_sum = 0
    ...
    def _eta_sample(self, sec: float, work: int, done_pl_add: int = 1):
        with self.clock_lock:
            ...
            s = float(sec or 0.0)
            if s > 0.0 and w > 0:
                if len(self.clock_recent) == self.clock_recent.maxlen:
                    os0, ow0 = self.clock_recent[0]
                    self.clock_recent_sec_sum -= os0
                    self.clock_recent_work_sum -= ow0
                self.clock_recent.append((s, w))
                self.clock_recent_sec_sum += s
                self.clock_recent_work_sum += w
    ...
    def _tick_runtime_clock(self):
        with self.clock_lock:
            ...
            base_done_pl = max(0, int(self.clock_base_done))
            base_done_work = max(0, int(self.clock_base_done_work))
            last_eta_sec = self.clock_eta_last_sec
            recent_sec_sum = float(self.clock_recent_sec_sum)
            recent_work_sum = int(self.clock_recent_work_sum)

        eta_txt = "--:--:--"
        rem_pl = max(0, total_pl - done_pl)
        rem_work = max(0, total_work - done_work)
        done_pl_delta = max(0, done_pl - base_done_pl)
        done_work_delta = max(0, done_work - base_done_work)

        eta_sec: Optional[int] = None

        if rem_work > 0 and recent_work_sum >= 500:
            rate = recent_sec_sum / max(1, recent_work_sum)
            eta_sec = int(round(rate * rem_work))

        if eta_sec is None and rem_work > 0 and done_work_delta > 0:
            rate = elapsed / max(1, done_work_delta)
            eta_sec = int(round(rate * rem_work))

        if eta_sec is None and rem_pl > 0 and done_pl_delta > 0:
            avg = elapsed / max(1, done_pl_delta)
            eta_sec = int(round(avg * rem_pl))

        if eta_sec is not None:
            eta_sec = max(0, eta_sec)

            if last_eta_sec is not None:
                cap = max(120, int(last_eta_sec * 0.20))
                eta_sec = min(eta_sec, last_eta_sec + cap)
                eta_sec = max(eta_sec, last_eta_sec - cap)

            with self.clock_lock:
                self.clock_eta_last_sec = eta_sec
