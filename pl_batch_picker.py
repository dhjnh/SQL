...
def score_pl_rows(rows: List[Dict[str, Any]], kb: Dict[str, Any]) -> Tuple[List[RowScore], int]:
    ...
    for i, row in enumerate(rows):
        ...
        gpg_last_token = gpg_tokens[-1] if gpg_tokens else ""
        ...
        if kb.get("pl_enabled"):
            ...
            for tok in gpg_tokens:
                for m_cat, m_sub, m_score in pl_map.get(tok, []):
                    # 按 KB 的最大分级刻度归一化（运行时从 load_kb() 注入）
                    scale_max = float(kb.get("pl_light_scale_max", 1.0) or 1.0)
                    if scale_max <= 0:
                        scale_max = 1.0
                    sc = _safe_float(m_score, 0.0)
                    if scale_max > 1.0:
                        # KB 用等级刻度(1~N)：用 N 归一化
                        s = max(0.0, min(1.0, sc / scale_max))
                    else:
                        # KB 已是 0~1：直接 clamp
            ...
            pl_delta = best_bonus if best_bonus > 0 else 0
...
class App:
    ...
    def run_main(self):
        ...
        with self.connect() as conn:
            ...
            queue_dirty = False
            # save_queue 改为每10个PL落盘一次以避免卡顿
            queue_save_every = 10
            last_queue_save_i = 0
            current_i = 0

            save_q: Queue = Queue(maxsize=1)
            save_stop = threading.Event()

            def _save_worker():
                while (not save_stop.is_set()) or (not save_q.empty()):
                    try:
                        path, snapshot, done_evt = save_q.get(timeout=0.2)
                    except Empty:
                        continue
                    try:
                        self._save_queue(path, snapshot)
                    except Exception as e:
                        self.log(f"WARN: async queue save failed: {e}")
                    finally:
                        done_evt.set()
                        save_q.task_done()
            ...
            def request_queue_save(block: bool = False):
                snapshot = [dict(r) for r in queue_rows]  # 必须快照
                done_evt = threading.Event()
                item = (queue_file, snapshot, done_evt)

                # 队列 maxsize=1：永远先丢弃旧请求，只保留最新快照
                while True:
                    try:
                        try:
                            old = save_q.get_nowait()
                            save_q.task_done()
                            try:
                                old[2].set()
                            except Exception:
                        ...
                        if block:
                            save_q.put(item)  # block=True：必须保证成功入队（不允许静默失败）
                        else:
                            save_q.put_nowait(item)  # block=False：尽量入队，不阻塞主循环
                        break
                    except Exception:
                        if not block:
                            done_evt.set()
                            return
                        time.sleep(0.05)  # block=True：持续重试直到成功入队

                if block:
                    done_evt.wait()

            def flush_queue_force():
                nonlocal queue_dirty, last_queue_save_i, current_i
                if queue_dirty:
                    self.set_stage("PL progress: save queue")
                    request_queue_save(block=True)  # 必须等待保存完成
                    queue_dirty = False
                    last_queue_save_i = current_i
            ...
            for i, q in enumerate(queue_rows, start=1):
                ...
                if queue_dirty and (i % queue_save_every == 0):
                    # save_queue 改为每10个PL落盘一次以避免卡顿
                    self.set_stage("PL progress: save queue")
                    request_queue_save(block=False)  # 异步，不阻塞主循环
                    queue_dirty = False
                    last_queue_save_i = i
            ...
            while True:
                try:
                    if getattr(save_q, "unfinished_tasks", 0) == 0:
                        break
                except Exception:
                    break
                if time.time() - t0 > 10:
                    self.log("WARN: queue save join timeout, exit without waiting further")
                    break
                time.sleep(0.1)
            try:
...
# 变更说明（本次稳定性修复）
# 1) 新增：App.ui_call_blocking，用于在主线程安全执行弹窗并阻塞等待结果。
# 2) 修改：run_main 内 flush_queue_force 去掉递归，改为 dirty 时一次性保存队列并清理标志。
# 3) 修改：run_main 中 Resume/_Mod exists 两处 messagebox 改为 ui_call_blocking 调用。
# 4) 修改：读取 queue_file 改为 with 打开并使用 newline=""，确保及时关闭句柄。
# 5) 新增：run_main 内异步队列保存线程（每10个PL异步保存），Stop/异常通过 flush_queue_force 同步落盘。
# 目的：防止递归崩溃、线程 UI 卡死、Windows 文件占用导致无法落盘续跑，并降低 save_queue 阶段阻塞。

# 变更说明（本次稳定性修复）
# 新增：PATCH 2（PLHash查询0行时fallback）、PATCH 5（队列保存等待加超时保护）。
# 修改：PATCH 1（after改主线程调度）、PATCH 3（队列生成过滤NULL PL键）、PATCH 4（SPN统计去掉LTRIM/RTRIM）、PATCH 6（避免KB重复加载）。
# 目的：消除Tk线程不安全、避免PL被误标DONE导致数据跳过、修复NULL键PL无法处理、减少无谓性能损耗、避免保存阶段无限卡死、降低启动重复开销。
