...
def build_pl_index(pl_df: pd.DataFrame):
    ...
    for pid, r in enumerate(pl_df.itertuples(index=False), start=0):
        ...
        if rem <= 0:
            continue
        vid = str(getattr(r, "VehicleId_Motor", ""))
        cat = str(getattr(r, "Category", "") or "")
        sub = str(getattr(r, "SubCategory", "") or "")
...
def eval_option(row_feat, leg_feat, pl: PLRec, std_pls_for_vid: Set[int]):
    ...
    return (lr, strong_overlap, legacy_score, legacy_conf, 0)
...
def ensure_rowid_and_index(conn, schema: str, table: str):
    obj = _qt(schema, table)
    obj_name = f"{schema}.{table}"
    idx = (f"IX_{table}__rowid")[:128]
    with conn.cursor() as cur:
        ...
        if not has:
            cur.execute(f"CREATE UNIQUE NONCLUSTERED INDEX [{idx}] ON {obj}([__rowid]);")
    conn.commit()
...
def _update_done_values(cur, schema: str, table: str, params_list: List[Tuple[str, str, int, int]], chunk_rows: int = CHUNK_VALUES):
    if not params_list:
        return
    head = f"UPDATE t SET t.[Category]=v.[Category], t.[SubCategory]=v.[SubCategory], t.[like]=v.[like] FROM {_qt(schema, table)} t JOIN (VALUES "
    tail = ") v([Category],[SubCategory],[like],[__rowid]) ON t.[__rowid]=v.[__rowid]"
    n = len(params_list)
...
def run_job(host: str, user: str, password: str, database: str, parts_full: str, pl_full: str, overwrite_done: bool, progq: "queue.Queue"):
    ...
    try:
        ...
        try:
            ...
            ensure_rowid_and_index(conn, done_schema, done_table)
            push("创建_Done表", 1, 1, "done table ready")

            need_parts = ["__rowid", "VehicleId_Motor", "SubCategory_GPG", "PNCDesc", "PartNumber", "PartDescription", "value1", "SearchPartNumber", "Category", "SubCategory", "like"]
            ...
            parts["value1_num"] = pd.to_numeric(parts.get("value1", 0), errors="coerce").fillna(0).astype(int)
            ...
            for j, (spn, rows) in enumerate(items):
                ...
                spn_max_v1[spn] = mv
                if (j % 200 == 0) or (j == M - 1):
                    push("统计SPN", j + 1, M if M else 1, f"{j + 1}/{M}")
            ...
            for idx, spn in enumerate(spns_3a):
                ...
                if (idx % 200 == 0) or (idx == len(spns_3a) - 1):
                    push("3A覆盖", idx + 1, len(spns_3a) if spns_3a else 1, f"{idx + 1}/{len(spns_3a)}")
            ...
            for idx, spn in enumerate(spns_3b):
                ...
                if (idx % 200 == 0) or (idx == len(spns_3b) - 1):
                    push("3B降级覆盖", idx + 1, len(spns_3b) if spns_3b else 1, f"{idx + 1}/{len(spns_3b)}")
            ...
            for idx, pid in enumerate(pl_ids_3c):
                ...
                if (idx % 200 == 0) or (idx == len(pl_ids_3c) - 1):
                    push("3C补满", idx + 1, len(pl_ids_3c) if pl_ids_3c else 1, f"{idx + 1}/{len(pl_ids_3c)}")
            ...
            with conn.cursor() as cur:
                _update_done_values(cur, done_schema, done_table, params, CHUNK_VALUES)
            conn.commit()

            X = len(spn_to_rows)
            Y = len(selected_pl_for_row)
            Z = len({spn_arr[rid] for rid in selected_pl_for_row})
            R = (Z / X) if X else 0.0
            summary = "\n".join(
                [
                    "Part 2) 总结（固定字段）",
...
class UI:
    def __init__(self):
        ...
        self.status = tk.StringVar(value="KB未加载")
        ttk.Label(self.root, textvariable=self.status, anchor="w").pack(fill="x", padx=12, pady=(2, 4))
    ...
    def login(self):
        ...
        if not host or not user:
            ...
            return
        try:
            self.status.set(f"KB loaded: {self.kb_version.get()} | 登录中：连接 master 拉数据库列表...")
            dbs = list_databases(host, user, self.pwd.get())
            self.db_combo["values"] = dbs
            if dbs:
                self.db.set(dbs[0])
            self.logged_in = True
            self.status.set(f"KB loaded: {self.kb_version.get()} | 已登录：数据库数量={len(dbs)}（请选择 Database）")
            self.db_combo.config(state="readonly")
            self.btn_tables.config(state="normal")
            self._log(f"Login OK. databases={len(dbs)}")
        except Exception as e:
            self.logged_in = False
            self.status.set(f"KB loaded: {self.kb_version.get()} | 登录失败")
            messagebox.showerror("Login failed", str(e))

    def _on_db_selected(self):
        if not self.db.get().strip():
            return
        self.status.set(f"KB loaded: {self.kb_version.get()} | 已选择数据库：{self.db.get().strip()}（请点“加载表列表”）")
    ...
    def load_tables(self):
        ...
        if not db:
            ...
            return
        try:
            self.status.set(f"KB loaded: {self.kb_version.get()} | 加载表列表中...")
            ...
            self.status.set(f"KB loaded: {self.kb_version.get()} | 表列表已加载：{len(tables)}（选择 PartsTable / PLTable）")
            self._log(f"Tables loaded: {len(tables)}")
        except Exception as e:
            self.status.set(f"KB loaded: {self.kb_version.get()} | 加载表失败")
    ...
    def start(self):
        ...
        self.running = True
        self.pbar["value"] = 0
        self.status.set(f"KB loaded: {self.kb_version.get()} | 运行中...")
